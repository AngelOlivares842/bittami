...
export default function BentoGrid() {
  ...
  return (
    <div className={`grid grid-cols-1 md:grid-cols-12 gap-5 max-w-7xl mx-auto p-6 auto-rows-[200px] transition-all ${isEzquizo ? 'scale-110' : ''}`}>
      ...
      {/* 2. OPERATIONS HUB (Información Restaurada) */}
            ...
            <div className="pt-4 border-t border-white/10 space-y-2">
              ...
              {['Viernes', 'Sábado', 'Domingo'].map(d => (
                  <div key={d} className="flex justify-between items-center bg-white/[0.02] p-2 rounded-xl border border-white/5">
                      ...
                      <span className={`text-[9px] font-bold tracking-widest ${isHacked || isEzquizo ? 'text-red-800' : 'text-bitta-pink'}`}>{isEzquizo ? "00:00" : "20:00H"}</span>
                  </div>
              ))}
            </div>
          </div>
      ...
      <Card className="md:col-span-3 flex items-center justify-center gap-4" isHacked={isHacked} isEzquizo={isEzquizo} onClick={() => window.open('https://instagram.com/bittami.vt')}>
        ...
        <span className="text-[11px] font-black uppercase">{isEzquizo ? "V01D" : "Feed"}</span>
      </Card>
      ...
      <Card className="md:col-span-6 flex items-center justify-between group h-full" isHacked={isHacked} isEzquizo={isEzquizo} onClick={() => window.location.href='mailto:bittami.mp@gmail.com'}>
        ...
        <div className={`px-3 py-1 rounded-md text-[9px] font-black uppercase tracking-tighter ${isHacked || isEzquizo ? 'bg-red-950 text-red-600' : 'bg-bitta-pink/10 text-bitta-pink'}`}>
            {isEzquizo ? "HELP" : "Contact"}
        </div>
      </Card>

      {/* --- EL SUSTO: Ventana de Error Falsa --- */}
      {isHacked && (
        <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[5000] w-80 bg-[#c0c0c0] border-t-2 border-l-2 border-white border-b-2 border-r-2 border-black p-1 shadow-[4px_4px_0px_#404040] font-sans">
          <div className="bg-[#000080] text-white px-2 py-1 flex justify-between items-center text-xs font-bold">
            <span>System Error</span>
            <button onClick={() => window.location.reload()} className="bg-[#c0c0c0] text-black px-1 border border-black text-[10px]">X</button>
          </div>
