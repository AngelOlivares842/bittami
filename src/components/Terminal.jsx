...
export default function Terminal() {
  ...
  const processCommand = (cmd) => {
    switch (cmd) {
      case 'ymir': // Nombre actualizado
        document.body.classList.toggle('mimir-mode');
        const isYmir = document.body.classList.contains('mimir-mode');
        showStatus(isYmir ? "MODO YMIR ACTIVADO: LUCES FUERA" : "MODO DIURNO RESTAURADO");
        break;
      ...
      case 'help':
        showStatus("COMANDOS: YMIR, BUNNY, HUG, EZQUIZO, CLEAR");
        break;
      ...
      default:
        if (cmd !== "") showStatus("ERROR: ESCRIBE 'HELP' PARA VER COMANDOS");
    }
  };
  ...
  return (
    <>
        ...
        <div className="fixed inset-0 z-[2000] flex items-center justify-center bg-black/90 backdrop-blur-md p-4">
          <div className="bg-[#1a1625] border-2 border-red-500 p-8 rounded-[3rem] max-w-md w-full text-center shadow-[0_0_50px_rgba(239,68,68,0.2)] animate-fade-in">
            ...
            <div className="flex gap-4">
              <button onClick={() => { toggleEzquizo(); setShowModal(false); }} className="flex-1 bg-red-600 hover:bg-red-700 text-white font-bold py-4 rounded-2xl transition-all uppercase text-[10px] tracking-widest active:scale-95"> Si, activar </button>
      ...
      {hugHearts.map(h => (
        <span key={h.id} className="fixed text-6xl animate-heart-burst z-[1001] pointer-events-none" style={{ left: `${h.left}%`, top: `${h.top}%`, animationDelay: `${h.delay}s` }}>💖</span>
      ))}
    </>
  );
}